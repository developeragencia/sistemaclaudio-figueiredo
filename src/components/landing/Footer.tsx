...
const Footer: React.FC = () => {
  return (
    <footer className="bg-gradient-to-br from-sky-900 to-sky-700 text-white py-10 relative">
...
const SocialIcon: React.FC<{ icon: React.ReactNode, href: string }> = ({ icon, href }) => {
  return (
    <motion.a
      ...
      target="_blank"
      rel="noopener noreferrer"
      className="bg-sky-600/50 hover:bg-sky-500 p-3 rounded-full transition-all duration-300 hover:scale-110 hover:shadow-lg hover:shadow-sky-500/20"
      whileHover={{ y: -3 }}
      whileTap={{ scale: 0.95 }}
    >
      {icon}
