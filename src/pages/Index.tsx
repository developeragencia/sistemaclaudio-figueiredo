...
const Index: React.FC = () => {
  ...
  // System features for the carousel
  const systemFeatures = [
    {
      title: "Recuperação de Créditos",
      description: "Identificação e recuperação automática de créditos tributários com algoritmos avançados que maximizam resultados.",
      icon: <Database className="w-10 h-10 text-blue-600" />
    },
    {
      title: "Relatórios Detalhados",
      description: "Visualização completa de dados para tomada de decisões estratégicas com relatórios personalizáveis.",
      icon: <FileText className="w-10 h-10 text-blue-600" />
    },
    {
      title: "Gestão de Clientes",
      description: "Gerenciamento completo da carteira de clientes com histórico, documentos e acompanhamento de processos.",
      icon: <Users className="w-10 h-10 text-blue-600" />
    },
    {
      title: "Análise Tributária",
      description: "Ferramentas analíticas para avaliação de cenários fiscais e identificação de oportunidades de economia.",
      icon: <BarChart3 className="w-10 h-10 text-blue-600" />
    },
    {
      title: "Segurança & Auditoria",
      description: "Proteção completa dos dados com trilhas de auditoria, autenticação em dois fatores e controle de permissões.",
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-white to-blue-50 overflow-hidden">
      ...
      <section className="py-20 md:py-28 relative overflow-hidden">
        ...
        <div className="absolute inset-0 bg-grid-blue-100/20 z-0"></div>
        
        <div className="container mx-auto px-4 relative z-10">
          <div className="max-w-6xl mx-auto">
            {/* Logo and Title Section */}
            <div className="flex flex-col items-center mb-16 text-center">
              <div className={`mb-8 transform-gpu transition-all duration-500 hover:scale-105 ${fadeInClass}`}>
                <AnimatedLogo size="large" animated={true} className="mb-6" />
              </div>
              <h1 className={`text-4xl md:text-5xl lg:text-6xl font-bold mb-8 bg-gradient-to-r from-blue-900 via-blue-700 to-blue-800 
                bg-clip-text text-transparent leading-tight max-w-4xl mx-auto transition-all duration-700 ${fadeInClass}`}>
                Sistema de Gestão Tributária para <span className="relative">
                  <span className="relative z-10">Escritórios de Advocacia</span>
                  <span className="absolute bottom-0 left-0 w-full h-2 bg-blue-200 opacity-50"></span>
                </span>
              </h1>
              <div className={`transition-all duration-700 delay-300 ${fadeInClass}`}>
                <Link to="/login">
                  <Button 
                    size="lg" 
            ...
            {/* Features Carousel */}
            <div className={`mt-16 transition-all duration-700 delay-500 ${fadeInClass}`}>
              ...
              <Carousel className="w-full max-w-5xl mx-auto">
                <CarouselContent className="-ml-4">
                  {systemFeatures.map((feature, index) => (
                    <CarouselItem key={index} className="pl-4 md:basis-1/2 lg:basis-1/3">
                      <div className="p-1">
                        <Card className="overflow-hidden h-full transition-all duration-300 hover:shadow-lg border-t-4 border-blue-500">
                          <CardContent className="p-6 flex flex-col h-full">
                            <div className="rounded-full bg-blue-100 p-3 w-16 h-16 flex items-center justify-center mb-4 mx-auto">
                              {feature.icon}
                            </div>
                            <h3 className="text-xl font-bold text-blue-800 mb-3 text-center">{feature.title}</h3>
                            <p className="text-blue-700 text-center flex-grow">{feature.description}</p>
                            <div className="flex justify-center mt-4">
                              <div className="flex items-center text-sm text-blue-600 font-medium">
                                <CheckCircle className="w-4 h-4 mr-1" /> Disponível
                              </div>
                            </div>
                          </CardContent>
                        </Card>
                ...
                <CarouselPrevious className="lg:-left-6 left-0" />
                <CarouselNext className="lg:-right-6 right-0" />
              </Carousel>
              
              <div className="flex justify-center mt-8 space-x-1 md:space-x-2">
                {Array.from({ length: 5 }).map((_, index) => (
                  <div key={index} className="w-2 h-2 rounded-full bg-blue-300 animate-pulse-slow" style={{ animationDelay: `${index * 0.2}s` }}></div>
                ))}
              </div>
            </div>
          </div>
